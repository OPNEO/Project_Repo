import random
def single_read_file():
    with open('diceoutput/single_dict.txt','r') as fileread:
        single_list=fileread.readlines()
        fileread.close()
        return single_list
def dual_read_file():
    with open('diceoutput/dual_dice.txt','r') as fileread:
        dual_list=fileread.readlines()
        fileread.close()
        return dual_list
history_of_single_dice=single_read_file()
history_of_dual_dice=dual_read_file()
...
while True:
    ...
    if continue_output==1:
        ...
        with open('diceoutput/dual_dice.txt', 'w') as file:
            ...
            file.close()
        print(f'Saved {len(history_of_single_dice)} single dice rolls.')
        print(f'Saved {len(history_of_dual_dice)} dual dice rolls.')
    else:
